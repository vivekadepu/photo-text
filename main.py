...
with st.sidebar:
    ...
    ocr_enhance = st.checkbox('Use OCR Enhance', value=False)
...
if img_files is not None:
    for img_file in img_files:
        ...
        st.markdown(f"### Generated Notes for {img_file.name}")
        st.text_area(label=f"Notes for {img_file.name}", value=notes_display, height=200)
        all_done += 1
        st.success('Done!')
